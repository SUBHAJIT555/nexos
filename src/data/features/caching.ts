...
export const cachingHero = {
  ...
  body: "Stop building separate caching logic for every provider. NexEco AI simplifies LLM caching through one integration, so your team can focus on building products.",
  bullets: [
  ...
  visualAlt: "NexEco AI Gateway cache routes requests to cached responses or an LLM provider",
};
...
export const cachingImprove = {
  ...
  intro:
    "NexEco AI works on two levels. Exact repeats are answered instantly by the Gateway's own cache, no provider call needed. Everything else is structured to make the most of the provider's own caching, so nothing gets processed at full price unnecessarily.",
  ...
  cards: [
    ...
    },
    {
      ...
      body: "Repeated context still gets processed at full price when caching is missing or misconfigured. NexEco AI helps identify repeated context and improve how effectively provider-side prompt caching is applied.",
    },
    {
      id: "providers",
      icon: "share" as const,
      title: "Handle provider differences",
      body: "Providers differ in how prompt caching is enabled, structured, and reported. NexEco AI manages those differences through one centralized integration, so you don't build a separate caching system for every provider.",
    },
    {
      id: "scale",
      icon: "scale" as const,
...
export const cachingTwoWays = {
  ...
  intro:
    "Caching saves money in two different places: at the Gateway and inside the provider. NexEco AI handles both, so you get the benefit either way. Here's how:",
  cards: [
    {
      id: "response",
      icon: "comments" as const,
      title: "Response caching",
      body: "The Gateway keeps its own cache of past requests and responses. When an identical request comes in again, NexEco AI returns the stored response directly, with no call to the provider at all. This is the fastest possible outcome: no generation, no wait, no cost.",
    },
    {
      id: "prompt",
      icon: "window" as const,
      title: "Prompt caching",
      body: "Prompt caching lives on the provider's side, reusing repeated prompt prefixes, like a shared system prompt, so requests that begin the same way skip reprocessing. NexEco AI makes sure your requests are structured to take full advantage of it, so you capture these savings without extra work.",
    },
  ],
};
...
export const cachingTackle = {
  ...
  intro:
    "NexEco AI turns caching from a per-provider chore into one integration your team sets up once. Instead of administering caching model by model, the NexEco AI Gateway manages it centrally, giving you visibility into cache effectiveness across every provider from one place.",
  cards: [
...
export const cachingResults = {
  ...
  body: "Nord Security is cutting costs and improving cache performance for their coding agents running on NexEco AI, without changing how their teams work. And the results speak for themselves:",
  stats: [
...
export const cachingCta = {
  ...
  body: "Turn repeated context into lower AI costs with NexEco AI LLM caching.",
  primary: { label: "Talk to Solutions Engineer", href: salesMailto } satisfies ProductLink,
...
export const cachingFaq: ProductFaqItem[] = [
  {
    id: "multi",
    question: "Does NexEco AI LLM caching work across multiple AI providers?",
    answer: faqParagraphs(
      "Yes. NexEco AI is model-agnostic by design, so caching applies across the different providers you connect through a single Gateway. Instead of managing caching provider by provider, you route your requests through one integration and let NexEco AI maximize cache hits across supported large language models, capturing savings across your entire AI stack rather than one model at a time.",
    ),
  },
  {
    id: "replace",
    question: "Does NexEco AI replace provider caching?",
    answer: faqParagraphs(
      "Partly – NexEco AI has its own AI Gateway cache that catches exact repeats before a request ever reaches the provider, on a hit, there's no call to the provider at all.\n\nThe AI gateway keeps its own cache of past requests and responses.",
    ),
  },
  {
    id: "separate",
  ...
  },
  {
    ...
    answer: faqParagraphs(
      "No. Developers don't need to rearchitect their applications or build provider-specific caching logic. Your requests keep flowing through the same single endpoint they always have, and NexEco AI handles caching underneath, so the cost savings and faster responses show up without any workflow changes. It's the same experience Nord Security had: better caching, zero changes required.",
    ),
  },
  {
    id: "secure",
    ...
    answer: faqParagraphs(
      "Yes. Caching happens within each provider's secure infrastructure, and NexEco AI never uses your data to train models. Cached requests are covered by the same protections as every other call through our platform for AI security.",
    ),
  },
];
